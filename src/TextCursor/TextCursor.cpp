...
void
TextCursor::updatePos(int line, int charIndex)
{
  _line = line;
  _charIdx = charIndex;
}
...
void
TextCursor::updateNumOfCharInLines()
{
  std::string currLine;
...
void
TextCursor::moveOneRight()
{
  if (_charIdx <
      numOfCharInLines[_line] - 1) // TODO make sure that you are staying within
...
void
TextCursor::setCursorPos(int line, int charIndex)
{
  updatePos(line, charIndex);
}

void
TextCursor::moveToNextLineStart()
{
  if (_line < _doc.getLineCount())
    updatePos(_line + 1, 0);
}
